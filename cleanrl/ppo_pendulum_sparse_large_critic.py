# ppo_pendulum_sparse_large_critic.py
"""
Moderately easier sparse Pendulum swing-up with a LARGE CRITIC (Width 128).
Designed to control for parameter count against DART (which uses 2x Width 64 Critics).
"""
import os
import random
import time
...
@dataclass
class Args:
    # UPDATED: Experiment name for tracking
    exp_name: str = "ppo_pendulum_sparse_large_critic"
    seed: int = 1
    ...
    # UPDATED: Run name
    wandb_run_name: str = "ppo_pendulum_sparse_large_critic"
...
class ModerateSparsePendulumReward(gym.Wrapper):
    ...
    - Otherwise reward 0 every step. If episode terminates without first-visit success,
      perform a terminal check using stricter thresholds.
    """
    def __init__(
        self,
        env,
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # --- LARGE CRITIC CONFIGURATION ---
        # DART uses 2 critics with hidden dim 64.
        # Here we use 1 critic with hidden dim 128.
        # Params Comparison (Hidden Layer): 
        #   DART: 2 * (64 * 64) = 8,192 parameters
        #   This: 1 * (128 * 128) = 16,384 parameters
        # This agent has nearly double the capacity of DART.
        
        self.critic = nn.Sequential(
            layer_init(nn.Linear(obs_shape, 128)),  # Increased from 64
            nn.Tanh(),
            layer_init(nn.Linear(128, 128)),        # Increased from 64
            nn.Tanh(),
            layer_init(nn.Linear(128, 1), std=1.0), # Increased from 64
        )
        
        # Actor remains standard size (64)
