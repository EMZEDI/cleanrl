# ppo_mc_critic_dm_control.py
# PPO with MC-Critic Ablation for dm_control benchmarks.
# Architecture: 128-128 hidden layers (Double size to match DART's parameters).
# Purpose: Ablation to show DART's gains vs a critic that learns purely from 
# Monte Carlo (MC) returns (regression) rather than TD/GAE targets.
import os
import random
import time
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # 1. We still calculate GAE advantages for the actor
            ...
            # 2. Compute exact Monte Carlo returns (Regression Target for Critic)
        ...
        # --- ABLATION FLATTEN: We use mc_returns instead of GAE returns for the value target
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # --- ABLATION VALUE LOSS: Train against b_mc_returns instead of b_returns
                newvalue = newvalue.view(-1)
                if args.clip_vloss:
                    v_loss_unclipped = (newvalue - b_mc_returns[mb_inds]) ** 2
                    ...
                    v_loss = 0.5 * ((newvalue - b_mc_returns[mb_inds]) ** 2).mean()
        ...
        # --- ABLATION METRIC UPDATE: Explained variance now checks against MC targets
        y_pred, y_true = b_values.cpu().numpy(), b_mc_returns.cpu().numpy()
        var_y = np.var(y_true)
